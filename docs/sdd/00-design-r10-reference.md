# Referência histórica: design r10 recebido da main

Este arquivo preserva integralmente a versão de DESIGN.md recebida no commit remoto `443930f`. As regras confirmadas posteriormente pelo usuário e a implementação atual estão no DESIGN.md principal, em ARCHITECTURE.md e em VALIDATION.md. O conteúdo abaixo registra planejamento, não comprova implementação.
